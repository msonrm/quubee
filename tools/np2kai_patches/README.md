...
| `07_cpu_mem_fastpath.patch` | **CPU 高速化の統合パッチ (第 1 弾 2026-07-10 + 第 2 弾 2026-07-11)**。実測: Suika3 (32bit PM) **1.39 倍** (11.2→8.1ms/frame)・Ray IV (16bit 実モード) **1.43 倍** (14.0→9.8ms/frame)。挙動不変 = screen hash 一致 + 回帰スイート全 PASS。内容: ① `ia32/cpu.h` に `qb_phys_read/write{8,16,32}(_codefetch)` — conventional (<0xA4000/0xA0000) + 拡張メモリ 2 窓 ([USE_HIMEM,EXTLIMIT16) / **[16MB,EXTLIMIT)** — DOS/4GW は 32bit コードをここに置く。実測 2.67 億回/600f の呼び出しが全部この窓) を直アクセス、他は `memp_*` へフォールバック。② `qb_codefetch{,_w,_d}` (inline 版 cpu_codefetch) + `ia32.mcr` GET_PC* / `interface.h` cpu_memoryread* 差し替え。③ `qb_vmemoryread/write/RMW_{b,w,d}` — !PM 分岐 (cpu_mem.mcr と逐語同一) をインライン化、cpu_mem.c は `QB_CPU_MEM_IMPL` ガードで除外。④ `qb_load_segreg` — 実モード/VM86 の segdesc_set_default を逐語インライン (Ray の LES 連発が主客)。⑤ `cpu.c` exec_allstep に 16bit 頻出 10 命令の直接ディスパッチ (call_indirect→直接呼び。関数は insttable と同一なので意味論不変)。⑥ `cpumem.c` に溢れアドレスの MB 帯ヒストグラム常設 (`np2kai_debug_memprobe(100+i/200+i)`、速度 triage 用)。意味論は upstream と厳密同一 (境界 -1/-3・A20・write8 の 0x457・descriptor 全フィールド)。代償: wasm 976KB→1.39MB (+42%、インライン展開。gzip 配信で大幅圧縮)。関連: CMakeLists の `USE_CPU_INLINEINST`/`USE_CPU_EIPMASK` 有効化。計測 `tools/bench_game.js` (Suika3) + `tools/bench_ray.js` (Ray)。**bench_frame.js は例外連発で longjmp を測ってしまい CPU 最適化 A/B に不適** |
