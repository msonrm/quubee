...
| `07_cpu_mem_fastpath.patch` | CPU メモリアクセスと命令フェッチの RAM fast path をインライン化 (Suika3 実測 **1.37 倍**、挙動不変 = screen hash 一致 + 回帰スイート全 PASS)。従来は 1 バイトごとに別翻訳単位の `memp_read8()` (if チェーン) を呼び、DOS/4GW がコードを置く 16MB 以上 (`CPU_EXTLIMIT` 窓) では 600 フレームに 2.67 億回の関数呼び出しが発生していた。`ia32/cpu.h` に `qb_phys_read/write{8,16,32}(_codefetch)` (conventional <0xA4000/0xA0000 + 拡張メモリ 2 窓 [USE_HIMEM,EXTLIMIT16)/[16MB,EXTLIMIT) を直アクセス、他は memp_* へフォールバック) と `qb_codefetch{,_w,_d}` (inline 版 cpu_codefetch) を追加。`ia32.mcr` の GET_PC* と `interface.h` の cpu_memoryread*/write* マクロを差し替え。`cpumem.c` に溢れアドレスの MB 帯ヒストグラム (`qbDebug` → `np2kai_debug_memprobe(100+i/200+i)`) を常設 (新規タイトルの速度 triage 用)。意味論は memp_* の先頭 fast path と同一 (境界 -1/-3・A20 マスク・write8 の 0x457)。代償: wasm +20% (976KB→1.17MB、フェッチサイトへの展開)。計測 `tools/bench_game.js` (bench_frame.js は例外連発で longjmp を測ってしまい不適) |
