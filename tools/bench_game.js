...
//
// 使い方: node tools/bench_game.js [ゲームディレクトリ] [multiple]
//   省略時は ~/suika3_audio/game-d (snapshot_test.js と同じ)。無ければ SKIP。multiple 既定 20。
...
const multiple = +(process.argv[3] || 20);
if (!fs.existsSync(dir)) { console.log('SKIP — ゲームディレクトリが無い: ' + dir); process.exit(0); }

(async () => {
    const m = await Machine.boot({ dir, multiple });
