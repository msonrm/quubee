...
- ✓ **② 同梱配線 + 音楽ポップアップ UI 完了 (2026-06-16、JS/HTML/asset のみ・Wasm 不変)** — `.M` を
  ...
  チェーン整合トリムで slot[N]=曲名/[N+1]=作曲/[N+2..]=コメントを抽出 (N=titleOffset、MC バージョン別に動的検出)。
  **東方旧作 BGM コーパス 45 本で全数検証** (`tools/pmd_meta_test.js`、ZUN 本人コメントまで復号)。
  **2026-06-30: titleOffset 動的検出を実装** — MC v4.8a+ は予約 3 スロット (PPZFile+PPSFile+PCMFile、ent[0]=0xFF)、
  v4.2a-v4.7x は 2 スロット (headerSize>=0x1A)、旧は 1 スロット。旧コードの `ent[3]` 固定が原因の「全部ずれる」報告を根治。
  合成テスト 3/3・東方コーパス 45/45 PASS。回帰ゼロ (pmd_test 2/0・touhou 4/0・batscript 51/0 等)。
...
- ✓ **画像/音楽 (.MAG/.PI/.M) の単体オープン (2026-06-25、JS のみ・Wasm 不変・実機確認はユーザー委任)** —
  閲覧専用形式 (`isPreviewOnlyName = isImageName || isMusicName`、テキストは対象外) は D&D/Open/＋Add のどれでも
  単体で開け、実行されないので束を壊さず重ねる (**非破壊** = `!append` でも closeBundle しない → 誤ドロップで
  前のゲームが消えない。空なら新規 1 個・ロード済みなら追加)。`encodeSjis` で SJIS 名化して /run 配置 (日本語名
  CG も可)・開いたら `openImage`/`openMusic` を自動で開く。Open accept に .mag/.pi/.m 追加。実行形式/書庫の D&D は
  従来どおり新規。詳細 CHANGELOG / [[project_readme_viewer_qol]] [[feedback_ui_design_principles]]
- ✓ **FMP (Guu) 音楽 + 「ちびおと」(86+ADPCM) 対応 — FMDSP の code 7 を根治 (2026-06-27、ブラウザ実機確認済)** —
  FMP の定番ビジュアライザ FMDSP.COM が `exited code 7` で動かない報告を根治。真因 = **FMDSP はドライバ常駐前提の
  ビジュアライザで自身はドライバを持たず、FMP/PMD 未常駐だと code 7 で終了**する。`FMP s` (サイレント常駐) を先に
  EXEC すれば起動。FMP は我々の 86 ボードを正しく認識 (`Found YM2608, INT5=IRQ12, Hooked INT d2h`) = ボード/IRQ は
  既定のまま完全一致 (音源側の作業不要)。`FMP s`→`PLAY *.opi` で FM 音楽が鳴る。何も同梱せず**ユーザー持ち込みを
  動かす**完全クリーン経路 ([[reference_fmp_vs_vgm_format_research]] の「クリーン同梱は不可」とは別物)。データ拡張子:
  `.mpi/.mvi/.mzi`=MML ソース、`.opi`(FM)/`.ovi`(ADPCM)/`.ozi`(PPZ)=コンパイル済み。**`.ovi` の ADPCM 声部を
  鳴らすため「ちびおと」(PC-9801-86 + ADPCM RAM = SOUND_SW 0x14) のセッション限定有効化を実装** =
  `np2kai_set_chibioto`/`qbDebug.chibioto(0|1)` (既定 OFF、0x14 は OPNA 実時間挙動を変えるので ADPCM が要る
  セッションだけ on。次 Run の reset で fmboard_reset がボード再 bind)。あわせて INT 21h **AH=0Eh**(Select Disk)/
  **AH=34h**(Get InDOS flag) を補完。回帰 `tools/fmp_test.js` 3/3 + 全テスト + bio100 triage 回帰ゼロ。詳細
  CHANGELOG / [[project_fmp_fmdsp]]。**2026-06-27 に実機確認 (FM/ちびおと/FMDSP 問題なし) を経て ちびおと既定 ON 化**
  (forceChibi=true)。あわせて FMDSP で音が詰まったため既定クロックを 27→20 に差し戻し (上のリファレンス機種表)。
  残: FMP レシピ自動検出+サイドバー tap 再生 (将来)
- ✓ **bound NE 実行ファイル (ぶろっくでポン brpn100.lzh) の黒画面を根治 — staged EXE で e_maxalloc を honor
  (2026-06-27)** — ユーザー報告「真っ黒・停止せず」。`brpn.exe` は bound NE (MZ スタブ + NE 本体、自前ローダ
  が自分の EXE を開いて NE セグメントを実モードへロード)。真因 = 我々が MZ `e_maxalloc` (=0xFFFF) を無視し
  `body+e_minalloc` だけ割り当てていたため空きが本体直上に残り、NE ローダの `AH=48h` 一時バッファが低位
  `0x020E` を返して **seg1 最終ロード先 0x121 と衝突** → 後続 read が `main()` を seg3 データで上書き → データへ
  call して `jmp far 0xb982:dc82` (VRAM 0xFF) で #UD ループ = 黒画面。修正 = loader-start EXE 経路で alloc_base を
  `body+clamp(e_maxalloc, e_minalloc, 空き)` に (e_maxalloc=0xFFFF→全メモリ所有=実 DOS と同じ→`AH=48h` が実機
  同様に失敗し衝突消滅。cstartup の `AH=4Ah` self-shrink 後は malloc/EXEC 通常動作)。**brpn がタイトル画面描画**
  (100% 非黒・6 色)。COM/EXEC 子/.bat は不変、bio100 31本は本修正あり/なし A/B で完全一致 (ALIVE21/CRASH0=回帰ゼロ)。
  bound NE / 自己展開型 EXE 全般に効く。回帰 `tools/exe_maxalloc_test.js`。詳細 CHANGELOG / [[reference_staged_exe_maxalloc]]。
  **ブラウザ実機 T3 確認済 (2026-06-27、ユーザー)**
- ✓ **常駐ソフト (FreeWay frway102.lzh) の VSYNC アニメが静止する件を根治 — 最上位 TSR の idle を IF=1 に
  (2026-06-27、ブラウザ実機確認済)** — FreeWay は VSYNC (PC-98 IRQ2=INT 0Ah) をフックして夜景/擬似 3D 道路を
  アニメーションさせる「3D ドライブ環境常駐ソフト」。真因 = `AH=31h` (TSR) は IF=0 で入るため、最上位
  プログラムが TSR したときの我々の idle (halt loop=`HLT;JMP`) が IF=0 のままで、`HLT` が VSYNC 割り込みで
  起きず常駐 ISR が一度も走らない (画面静止)。修正 = `qb_dos_signal_tsr` の最上位ブランチで `CPU_FLAG |= I_FLAG`
  (`native/dos_loader.c`)。worker tick は exited を見ず run_frame を回し続けるので IF=1 さえ立てば ISR が VSYNC
  毎に動く。PMD 音楽 TSR の「IF=0 だと最初の1音だけ」と同型。EXEC 子 TSR (Ray RIN.COM・.bat ドライバ) は親復帰の
  別経路で不変・正常終了の halt loop (IF=0 凍結) も不変。回帰 `tools/tsr_vsync_test.js` (corpus・不在 SKIP)。
  bio100 ベースライン維持 (CRASH=0)・int27_tsr/touhou 4/4 等 PASS。詳細 CHANGELOG / [[reference_int27_oldstyle_tsr]]
- ✓ **仮想 30行BIOS (qbDebug.lines30) — VZ で 30 行テキスト表示 (Phase 1、2026-06-28、ブラウザ実機確認済)** —
  実機 30行BIOS/30行計画 は NEC CRT-BIOS ROM をパッチして常駐する方式で、ROM を積まない QuuBee には常駐できない
  (30BIOS.COM は「対応していないDOS」で自滅 = neccheck 壁)。そこで「30BIOS が常駐済みの最終状態」を HLE が用意する
  アプローチ (30TECH.DOC を一次情報に)。`qbDebug.lines30(1)`/`np2kai_set_lines30` で ON、既定 OFF=IVT[0x18] 不変=
  ゼロ回帰。loader-start が IVT[0x18] を新トランポリン 0xFEEC0 へ横取りし `qb_dos_int18_hook` が 30BIOS-API
  (BX=0xC0A3 の AH=0Bh インストールチェック〔AL bit6 常駐/bit4 拡張・ES:DI の 30BIOS_EXIST=0→1 書換〕、AX=FF00/03/04/05)
  を処理、それ以外は `bios0x18` へパススルー。GDC は INT 18h AH=30h 合成 (AL=0x0C/BH=0x32) で 640×480・30 行へ
  (EXTs21 と同じ拡張 CRT-BIOS 経路)・tty を 30 行 (TEXT_ROWS 実行時変数化)・0:0712=29/0:0713=1 に。黒画面根治 =
  256 色 (PEGC) を戻し + AH=0Ch でテキスト表示再開。core 改変は bios.c の case 0xFEEC0 のみ
  (01_dos_loader_hooks.patch 再生成)。Canvas-98 のグラフィックキャンバス (Phase 2) は調査の上見送り =
  PC-98 16 色 VRAM が 400 ライン分しか無く 480 グラフィックが物理的に不可。検証 `tools/lines30_test.js`・回帰ゼロ。
  詳細 [docs/30line_spec.md](docs/30line_spec.md) / [[reference_pc98_30line_crtbios]]
- ✓ **font.bmp の漢字の縦位置ズレを根治 — irori 版へ乗り換え + 再生成パイプラインを vendoring (2026-06-28、
  JS/asset のみ・Wasm 不変・ブラウザ実機確認済)** — ユーザー報告「縦位置が微妙にズレている」を根治。素材の
  SimK 版 font.bmp は寄せ集めでベースライン不揃い (漢字に最大 1px のランダム縦ズレ)。**irori 氏 np2-wasm
  `adjust-fontbmp` の手法 (二バイト JIS を東雲フォント=Public Domain で上書きして縦位置正規化) を採用**し、
  我々の区8罫線を `gen_keisen_glyphs.py` で再注入。再生成一式を `tools/font_build/` に vendoring
  (`makefont.cjs`=BSD-3・東雲 BDF=PD・base=SimK 修正BSD、出力は irori 版と md5 一致)。NEC 機種依存文字
  (区9-13)・ANK は保持 = 乗換えでも PC-98 固有グリフ不変。**史実訂正**: 区8 (全角罫線) は JIS X 0208-1983 追加で
  JIS78 ベースの NEC PC-98 実機 ROM には無く (実機の箱描画は区9-11 NEC 半角側)、SimK/irori が区8 空なのは
  faithful、我々の区8注入は JIS83 前提ソフト (VZ の GAME 等) を動かす意図的拡張。VZ で「閉」等のベースライン
  整合・罫線無崩れを実機確認。font.bmp md5 adf8d8→e6a4ab・デプロイ済。詳細 CHANGELOG / [[reference_fontbmp_glyph_gaps]]
- ✓ **WinDy (wd113) の画面崩れを根治 — NEC ANSI CSI 私的マーカ '>' をパラメータの後でも受理 (2026-06-28、
  native + Wasm・ブラウザ実機確認済)** — X 動作報告から。WinDy (テキストエディタ兼ファイラ環境) はファンクション
  キー行制御を `ESC[1>h` / `ESC[1>l` という **NEC PC-98 形式 (数字 → '>' の順)** で送るが、我々の tty CSI パーサ
  (`native/dos_int21.c`, TTY_CSI) は '>'/'?' を「'[' 直後 (数字より前)」でしか私的マーカ扱いせず、数字後の '>' を
  終端文字に誤認 → 続く 'h'/'l' を素の文字として描画 (メイン画面左上の謎の 'h' = 崩れの真因)。修正 = '>'/'?' を
  **位置によらず priv 記録** ( `ESC[>1h` と `ESC[1>h` を同一視。標準系不変 = ゼロ回帰)。回帰 `tools/csi_priv_test.js`
  (合成 ESC・corpus 不要)。あわせて **OTENKI (おてんきぶっく'91、書庫未入手・スクショのみ) の容疑だった FCB I/O を
  全数調査** → bio_100 全 30 本で FCB レンジ `UNIMPL AH` ゼロ (KANI123 も不使用) = 「崩れ=FCB 未実装」線を否定。
  バイナリ無し動作報告は `printErr→console.warn` (emu-worker.js) でブラウザコンソールに UNIMPL が出るのを診断に使う。
  詳細 CHANGELOG / [[reference_pc98_nec_ansi_csi_priv]] [[reference_browser_console_diagnostic]]
- ✓ **BEEP 音量を既定 ~4x にブースト + AMEL の MIDI モードを根治 (2026-06-28、native+JS・ブラウザ実機確認済)** —
  amel133 (AMEL) 作者の「FM は良いが BEEP が聴こえない・すごく小さい」報告から FM/BEEP/MIDI バランスを headless
  実測。FM↔MIDI は ~5dB で問題なし、**BEEP だけ FM/MIDI より 18-23dB 低い** (真因 = np2kai の `beepcfg.vol` が
  0..3 の 4 段階しか持たず矩形波が peak 2048(-24dBFS)で頭打ち)。**`vol_master` が fmgen FM/TSF MIDI に効かず
  BEEP/ADPCM/PCM だけに効く性質**を使い、ADPCM/PCM を相殺しつつ BEEP だけを既定 ~3.8x(+11.7dB) に持ち上げ
  (`np2kai_set_beep_gain`、`np2kai_create` が既定適用、実行時 `qbDebug.beepgain(x)`)。FM/MIDI/ADPCM は実測で完全
  不変・純設定の上限 383%。回帰 `tools/beep_gain_test.js` 3/3。**あわせて AMEL の MIDI 無音を根治** = MIDI 結線の
  on-demand 判定 `usesMidi` が `MIDDRV/MMD` 系しか見ず **AMEL 同梱の `midrv.com` (MIDDRV とは別物) が漏れていた**
  → `batscript.js` の `MIDI_DRIVER_NAMES`/`DRIVER_NAMES` に `midrv` 追加で MPU98II attach + RS-MIDI 結線が発火
  (JS のみ・Wasm 不変)。**BEEP・MIDI とも実機確認済 (ユーザー「綺麗に鳴ってる」)**。詳細 CHANGELOG /
  [[reference_beep_volume_boost]] [[project_midi_status]] [[project_bat_launcher_corpus]]
- ✓ **INT DCh CL=10h (NEC PC-98 DOS コンソール BIOS) を実装 — 蟹味噌の左上テキスト残留を根治 (2026-06-29、
  native + Wasm・ブラウザ実機確認済)** — おてんきぶっく'91 (OTENKI) のスクショ報告 + lpproj 氏の指摘 (gist:
  FreeDOS(98) INT DCh サポート状況) を発端に、長年未解決だった蟹味噌 (KANI123) の「起動直後に左上へ
  『KANI.SCRを作成します』が残り続ける」を根治。真因 = INT DCh は CL (サブ機能) で枝分かれし、我々は VZ/JED 用に
  CL=0Ch/0Dh (キー定義) のみ実装、**CL=10h (文字・画面制御) を no-op で握り潰していた**。蟹味噌は CL=10h AH=01h で
  `ESC[2J`(画面消去) 入り文字列を発行するが効かず残留。修正 = `qb_dos_intdc_hook` に CL=10h を実装し新規描画ゼロで
  既存 tty へ橋渡し。2026-06-06「直書きでクリアしない」結論は誤り (当時 INT DCh ハンドラ未存在で観測不能)。**SimK 氏
  TEXTTEST の同梱 .asm が実 .COM と食い違い、実 dump が正典** (asm を信じて 2 度誤読した教訓)。回帰
  `tools/intdc_screen_test.js`。**追補同日**: lpproj gist が ○ とする残り **AH=04-09 (カーソル ↑↓→← 移動)・
  0Ch/0Dh (行挿入/削除、`vram_insert_lines`/`delete_lines` 追加で `ESC[nL/nM` 未発火ギャップも解消)・0Eh (漢字-
  グラフモード=SJIS 直接解釈なので no-op が忠実)** も実装し CL=10h を端まで完成 (AH=02h で 0:071Dh 同期も追加)。
  回帰 `tools/intdc_cursor_test.js` 14/14。bio100 ベースライン一致 (ALIVE21/CRASH0)。詳細 CHANGELOG /
  [[reference_int_dch_cl10_screen]]。YY は INT DCh 不使用=別系統 (VRAM 直書き疑い・後回し)
- ✓ **ESC/CSI の「消費するが無視」系を根治 — DSR / カーソル保存復元 / 逆インデックス (2026-06-29、native + Wasm)** —
  SimK 氏 (NP21/W 作者) の TEXTTEST 群 (DOSTXT/PC98ESC/PC98ADV1-2/PC98HW/PC98DCH、np21w 正解スクショ付き) を
  ヘッドレス全数実行して未対応を洗い出し。**INT 21h/INT DCh の API レベルは未対応ゼロ**だったが、tty/CSI が
  受理だけして無視する ANSI シーケンスが数点あり (診断ログに出ない)、特に **ESC[6n 無視で PC98ADV1 がハング**して
  いた。修正 (`native/dos_int21.c`): ① **ESC[6n (DSR カーソル位置レポート)** = 応答 `ESC[<row>;<col>R` を入力 FIFO
  (`qb_dos_inject_input`・ホスト IME 経路流用) へ注入 (全読み取り口へ届く)、② **ESC[s/ESC[u** = カーソル+属性の
  保存/復元 (PC-98 CON は属性も)、③ **ESC M/D/E** = reverse index/index/next line (生 ESC を破棄していたのを実装、
  ESC M 上端は `vram_insert_lines(0,1)` 逆スクロール)、④ **ESC = l c** = VT52/PC-98 直接カーソル位置指定
  (`TTY_ESC_EQ_ROW/COL` 追加、`row=byte-0x20`。**ブラウザ実機でユーザーが PC98ESC PAGE 5 の上半分崩れを発見**して判明
  — 旧実装は ESC= を破棄し位置バイト 2 つを文字描画していた)。ESC[r (スクロール領域) は全画面固定のため意図的に未対応。
  **TEXTTEST 全 6 本が完走** (修正前は ESC[6n でハング)・全 6 本 UNIMPL ログゼロ。回帰 `tools/esc_seq_test.js`
  10/10 + 既存全 PASS + bio100 ベースライン一致 (ALIVE21/CRASH0)。np21w は強い参照だが絶対の正解ではない点に留意
  (API レベルの結論はログ+ソースで np21w 非依存)。[[reference_pc98_nec_ansi_csi_priv]]
- ✓ **グラフ文字モード (ESC)0/)3・INT DCh AH=0Eh) + SGR 2 縦線属性を根治 (TEXTTEST 突合、2026-06-29、native + Wasm)** —
  np21w 正解スクショと QuuBee を全 6 本・全ページ突合 (比較 PNG 生成)。**DSR/ESC=/INT DCh AH=0Ch/0Dh の先日修正が
  np21w とグリフ/バイト一致**を確認の上、残差 2 系統を根治: ① **グラフ文字モード**=「漢字 2 バイト結合の ON/OFF」。
  ESC)3/DX=3 で各バイトを ANK 1 文字として描く (実機: "テ"=83h 65h の 65h が 'e' で出る)。`g_graph_mode` + `ESC) n`/
  `ESC( n` 消費の `TTY_ESC_DESIG` 状態を追加し**末尾 `0`/`3` の文字漏れ ("テスト0/3") を根治**。INT DCh AH=0Eh を
  no-op から実装へ (旧「no-op が忠実」は誤り)。② **SGR 2 縦線属性のブロック化け**=np2kai maketext は属性 `0x10` を
  `gdc.mode1` bit0 (DEGB=簡易グラフィック) で二義解釈 (DEGB=0→縦線/=1→2x4 ブロック)。QuuBee は POST 既定 0x99
  (DEGB=1) を引き継ぎブロック化けしていた → `qb_dos_tty_reset` で **DEGB OFF** (実 MS-DOS 起動状態の再現・faithful)。
  回帰 `tools/graph_mode_test.js` 新設 + `sgr_test.js` に DEGB=0 ガード。全テスト/touhou 4/4/bio100 (ALIVE21/CRASH0)
  回帰ゼロ。**未対応に留めた差** (ユーザー判断): fkey 表示行と 24 行スクロール領域・20 行モードのセル高・blink 位相差。
  [[reference_int_dch_cl10_screen]] [[reference_pc98_nec_ansi_csi_priv]]
- ✓ **CON の IOCTL device info を 0x80D3 に修正 — YY のオープニングが入力前に出ない不具合を根治 (2026-06-30、native + Wasm)** —
  bio_100「ある勇者の憂鬱」(YY、TurboC 製テキスト ADV) が起動直後に真っ黒で入力待ち。**前日「getch 先読みで faithful・
  直す対象なし」と結論したのは誤り**で、ユーザーが公式レビュー (read-then-choose な ADV と明記) を根拠に再指摘。真因 =
  **AH=44h AL=00 (Get Device Info) が CON に不完全な 0x82 を返し**、欠けた **bit6 (0x40)** で TurboC が stdout を
  「リダイレクト先ファイル」と判定→ **full-buffer 化**。`getch()` (AH=07h・stdio を flush しない) で待つ YY の
  オープニングが stdio バッファに滞留したまま入力まで画面に出なかった。決定的証拠 = 入力前にプロンプト文字列が
  EXE 原本 1 回に対しメモリ上 2 箇所 (=stdout バッファのコピー)。修正 = CON (handle 0/1/2) に実機値 **0x80D3** を返す
  (`native/dos_int21.c`)。**入力ゼロで YY のオープニング全文+「[ Y)es or N)o ? ]」が表示**・triage colors 1→2。TurboC 系の
  printf→getch 型全般に効く。回帰 `tools/dev_info_test.js` + touhou 4/4 + bio100 (ALIVE21/CRASH0) ゼロ。教訓: 「getch 前に
  AH=40h が無い」≠「未 printf」(full-buffer なら溜まるだけ)。[[reference_dos_con_device_info_buffering]] [[feedback_hle_honest_failure]]
- ✓ **AH=3Fh handle 0 (STDIN) の cooked 行入力を実装 — getchar/scanf/gets が全滅していた stdin バグを根治 (2026-06-30、native + Wasm)** —
  SimK 氏が X で「stdin がおかしいかも」と指摘し、TEXTTEST(2) に **PC98IN.COM (入力テスト)** を追加 (AH=01/06/07/08/0A/0B/0C/3F・
  np21w 正解 9 枚)。突合の結果、AH=01/07/08/0A/0B/0C は正常 (PC98IN1=AH=01 が np21w とコード一致) で、**唯一 AH=3Fh が
  `fh_get(0)=NULL` で STDIN 読みを AX=6 エラーに**していた (標準ハンドル 0-2 未登録・AH=40h は h=1/2 を tty へ分岐するのに read 側に
  STDIN 分岐が無い非対称)。これで **TurboC の getchar/scanf/gets が全滅**。修正 = handle 0 を **cooked 行入力** (`int21_3f_read_stdin`、
  Enter 待ち・エコー・BS・末尾 CR LF) に分岐 (`native/dos_int21.c`)。np21w (PC98IN8) 通り "Hello"+Enter → 48..6F 0D 0A (count=7)。
  回帰 `tools/stdin_read_test.js` ("Hi"→count=4・48 69 0d 0a) + touhou 4/4 + bio100 (ALIVE21/CRASH0) ゼロ。[[reference_dos_con_device_info_buffering]]
- ✓ **AH=3Fh cooked 行入力の 3 点修正 (2026-06-30、native + Wasm)** — コードレビューで発見した正確性バグ 3 点を根治:
  ① **拡張キー NUL** — 矢印/Fn が `kb_get_word()` から `0xXX00` で返ると `ch=0x00` がバッファ書き込みガードを通過していた
  (AH=0Ah 経路には同問題なし)。`ch==0x00` なら `continue` で修正。② **SJIS BS 孤立バイト** — BS で `len--` を 1 回だけ行うと
  SJIS トレイルのみ論理削除されリードバイトが残留→不正 SJIS として返る。`peek8(dst+len-2)` が SJIS リードバイト範囲
  (0x81-0x9F/0xE0-0xFC) なら `del=2` で 2 バイト同時削除+エコー 2 回分。③ **want==1 で文字が全て捨てられる** — 容量ガード
  `len+2 < want` は `want=1` で常に false となり文字が書けない。早期リターン条件を `if (want < 2)` に拡張 (AX=0, CF=0)。
  回帰 `tools/stdin_read_test.js` / `tools/stdin_partial_line_test.js` (新規：分割到着・複数フレーム跨ぎを固定) / 全回帰 PASS。
