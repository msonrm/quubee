...
- ✓ **T6 Ray IV (2026-05-31)** — 起動・**RIN.COM 自動常駐 (EXEC COM 子 + AH=31h TSR)**・FM 音楽・
  メニュー描画まで動作。**当時の「画面黒のまま」はデータファイル未指定が原因と 2026-06-07 に判明** —
  `RAY SILK_FLD.RAY` 等を渡せばオープニング表示まで動く (罫線崩れも同日根治、下記参照)
...
- 次: Phase 3 一区切り。残は Ray の音楽再生まで通すか (Phase 4 候補)、bio 100% の残 EXIT 4 本 (CZ/GGL2/GS/OZ) と T3 ブラウザ確認
- ターゲット: フロッピーベース・2D・〜1998 年の同人/フリーソフト (期待カバー率 80〜90%)
- テストスイート: さめがめ ✓ / ザルバール ✓ / Super Depth LZH ✓ / Ray ✓(データ指定で表示・罫線根治) / うさちゃん列車 ✓
