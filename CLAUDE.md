...
- ✓ **AH=3Fh handle 0 (STDIN) の cooked 行入力を実装 — getchar/scanf/gets が全滅していた stdin バグを根治 (2026-06-30、native + Wasm)** —
  ...
  回帰 `tools/stdin_read_test.js` ("Hi"→count=4・48 69 0d 0a) + touhou 4/4 + bio100 (ALIVE21/CRASH0) ゼロ。残=拡張キーは未比較
  (np21w 参照が通常キーのみ)。[[reference_dos_con_device_info_buffering]]
