...
- ✓ **VZ Editor 対応 — テキストエディタ互換クラスの実証 (2026-06-20、ブラウザ実機確認済)** — PC-98 版 VZ Editor
  ...
  残=JED 別機構 (要調査)・VZ の fkey 行ラベル表示 (装飾)。詳細 [[reference_msdos_player]] [[project_text_editor_class]] / CHANGELOG
...
- ✓ **サブディレクトリ起動時の CWD 設定で Super Depth を救済 (2026-06-22、ブラウザ実機確認済)** — ゲームが書庫内の
  ...
  no-op (回帰なし)。回帰 `tools/subdir_cwd_test.js` (6/0) + 既存全 PASS + bio100 triage ベースライン一致。残: 日本語名
  サブディレクトリは SJIS トレイル 0x5C の誤分割可能性 (ASCII フォルダ名は完全動作)。詳細 CHANGELOG
