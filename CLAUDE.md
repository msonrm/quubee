...
- ✓ **FMP (Guu) 音楽 + 「ちびおと」(86+ADPCM) 対応 — FMDSP の code 7 を根治 (2026-06-27、ブラウザ実機確認済)** —
  ...
  **AH=34h**(Get InDOS flag) を補完。回帰 `tools/fmp_test.js` 3/3 + 全テスト + bio100 triage 回帰ゼロ。詳細
  CHANGELOG / [[project_fmp_fmdsp]]。残: ちびおと既定 ON 化 (条件付き保留)・FMP レシピ自動検出+サイドバー tap 再生 (将来)
...
| EXTMEM | 32MB |
| クロック | multiple=27（≈486DX2-66、ZUN 推奨環境相当）。2026-06-26 に 20≈49MHz から引き上げ（実機で音切れ無し確認）。bridge.js が起動時に emu.setClockMultiple(27) で適用し np2cfg.multiple に保持。autoclock は既定 OFF |
