...
- **検証**: `tools/diskimage_test.js` (Node) で raw/d88/fdi の3経路バイト一致・4階層再帰・非FAT判定 = pass 27/0。
  DCP/DCU はサンプル未入手で実バイト照合のみ未 (実装済)。
...
- **要: ブラウザ実機での目視確認** (ドロップ→一覧→フォルダ移動→Run)。コア/ロジックは Node 検証済。
