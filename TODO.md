...
- [ ] (将来/Phase4) Bio 100%「蟹味噌」: 左上「KANI.SCRを作成します」が起動〜本編 HUD まで残留。
      **2026-06-06 根因解明: QuuBee のバグではない**。kani.exe が起動時に text 面を一度クリア→row0 に直接
      書込→以降クリアせず、本編 HUD を**位置指定で重ね書き**(隙間は空白前提)。隙間に起動メッセージ漢字が
      残り score に被る。描画/dirty-flag/textdisp OFF/secret 属性/SAD 幾何/first-run/LZEXE(kanipic は LZEXE
      圧縮だが**正常展開・タイトル完璧描画**)を全否定済 (CHANGELOG 2026-06-06)。残り = 実機が遷移時にクリア
      するか初回限定で実機も被るか。断定には kani.exe 逆アセン or ブラウザ遷移時 `qbDebug.textVram()` 観測が
      必要 (headless は本ゲームの入力が不安定で本編遷移を踏めず)。**美観のみ・ゲームはプレイ可能。**
