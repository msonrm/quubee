...
- **起動 .bat の ② (線形列) 経路を ③ (文インタプリタ) に統合する** (2026-06-23、Exhibit B)。現在 .bat の起動は
  ...
  **【2026-07-05 追記】テストハーネスも ② の C API を直接使っている**: `tools/fmp_test.js` と
  `tools/adpcm_beepgain_test.js` が `np2kai_dos_stage_script` を呼ぶ (FMP s → PLAY の 2 コマンド線形列)。
  ② を撤去するならこの 2 本の stage_batch (`C\t…` ワイヤ形式) への置き換えも必要 (機械的・見積に小上乗せ)。
