...
- [ ] **快適化: `np2cfg.multiple` 引き上げ** (現 20、仕様の理想 42)。-O3 で 38% 余裕ができた。
      ただし fmgen は重いので **FM 再生中の余裕を `bench_frame.js` 改で再計測**してから安全に上げる
- [ ] **メモリ: XMS/EMS HLE** — 640KB の壁の外へ。既存 32MB 拡張メモリに載せる (深め、`dos_int21.c`)
