...
- **JED (jed194n.lzh) のカーソルキー = 根治済 (2026-06-24)**: 真因は INT DCh setkey の **1 キー単位 API**
  ...
  → **解消見込み (2026-07-02)**: QB 製アプリ ERR=5 根治で tty_store_cursor が GDC ハードウェアカーソル
  (CSRW) を論理カーソルに追従させるようになり、実 DOS CON 同様 HW カーソルが常に正しい位置に置かれる
  (JED はそれを当てにする作りだった)。要ブラウザ実機確認。詳細 CHANGELOG 2026-07-02。
