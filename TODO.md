...
- ✅ **キーリピート (押しっぱなしの連打) — 案A 実装 (2026-07-11、ブラウザ実機確認待ち)**。
  ...
  同一呼び出し内で完結するため無傷。**残 = ブラウザ実機確認**: ①エディタ (VZ/JED) でカーソル/
  文字が押しっぱなしでリピートする ②東方/Super Depth の押しっぱなし移動が滑らかなまま。
  駄目なら revert か案B/キー限定へ。以下は当時の調査メモ (経緯):
