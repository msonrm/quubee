...
- ✓ **一括テストランナー `tools/run_tests.js` (2026-07-11)**: `tools/*_test.js` 全 69 本を
  1 コマンド・並列 (既定 min(8,CPU))・個別タイムアウト (既定 300s) で回し PASS/SKIP/FAIL/TIMEOUT に
  分類する。`node tools/run_tests.js [filter] [--jobs N] [--timeout S] [--list]`。全緑以外は exit 1、
...
- ✓ **④ MCP アダプタ `tools/mcp/server.js` — v1 完成 (2026-07-11、ユーザー判断 = 対話セッション型 +
  ...
  **✓ 出力整合レビュー (2026-07-17 = 0.4.0)**: CLI `frames`→`frame` 改名 (到達位置で MCP と
  ...
  `seconds`→`audioSeconds`。意図差 (tier=classify のみ等) は README に明記。**✓ 0.4.0
  publish 済 (2026-07-17、latest=0.4.0 をレジストリから install し直して 18 PASS + 新スキーマ
  確認) = MCP 関連の宿題ゼロ**。
...
5. ✅ **[2026-07-05 修正済]** **音楽セッション初回スワップで残響対策が空振り (off-by-one)** — 初回 pass は stageMusic →
   ...
   → 修正済: フラグ倒しを stageMusic へ移動。ブラウザ実機確認済 (CHANGELOG 2026-07-05)。
6. ✅ **[2026-07-05 修正済]** **Stop 後に注入エンジン PMD86.COM/PMP.COM が一覧に出現** — resetToIdle が
   `hideEngineFiles=false` (bridge.js:1127) を同期で倒した後、非同期の onExit → syncRunDir が
   フィルタ無しで scanRun し、/run 残置のエンジンが「新規ファイル」として一覧へ。
   → 修正済: stopPolling が onExit の Promise を返し resetToIdle が await してからフラグを
   倒す順序に。ブラウザ実機確認済 (CHANGELOG 2026-07-05)。
...
14. ✅ **[2026-07-05 修正済]** **パッド押しっぱなしでタブ切替するとゲスト側キーが押されたまま自走** — blur ハンドラ
...
- **JED (jed194n.lzh) のカーソルキー = 根治済 (2026-06-24)**: 真因は INT DCh setkey の **1 キー単位 API**
  ...
  → **✅ 解消・ブラウザ実機確認済み (2026-07-02)**: QB 製アプリ ERR=5 根治で tty_store_cursor が
  GDC ハードウェアカーソル (CSRW) を論理カーソルに追従させるようになり、実 DOS CON 同様 HW カーソルが
  常に正しい位置に置かれる (JED はそれを当てにする作りだった)。ユーザーが JED でカーソルが動くことを
  実機確認。詳細 CHANGELOG 2026-07-02。
...
- **CON スクロール範囲ワークエリア 011Eh (top) / 0112h (bottom) の尊重** (2026-07-03、SimK PC98WORK
  PAGE6 で判明・ユーザー判断でゆるい TODO)。実 PC-98 DOS は DOS3/DOS6 とも LF スクロールがこの窓
  [top..bottom] の中だけで回る (窓外は不動)。我々は全画面スクロール固定。既定値=全画面なので通常は
  差が出ないが、窓を直書きするソフト (corpus では未遭遇) と将来の fkey 行表示 (bottom=23 連動) で必要。
  やるなら LF スクロールだけでなく ESC[nL/nM・INT DCh 行操作・cooked エコーの全経路を一貫させること
  (LF だけ窓対応にすると実 DOS に無い折衷が生まれる)。カーソルが窓外にいる時の LF 挙動などリファレンス
  未収集のコーナーがあるので、実装時は SimK 氏系テストでの裏取り推奨。エッジの意味論と回帰リスクの
  評価は 2026-07-03 の会話ログ参照 (①〜④=クリア/属性/保存カーソル slot は実装済・conwork_test.js)。

- ✅ **キーリピート (押しっぱなしの連打) — 案A 実装・ブラウザ実機確認済み (2026-07-11)**。
  bridge.js keydown が OS オートリピート (tap.repeat) もゲストへ転送するように変更 (JS のみ・
  Wasm 不変・即 revert 可)。C 側前提は恒久回帰 `tools/keyrepeat_test.js` (7 PASS) で型に封じた:
  keystat_down の再 down = break+make (新規ストローク) / 保持だけでは無音 (自前タイプマチック
  keyrepeat_enable は既定 OFF = 二重生成なし) / 修飾キーは KBEX_NONREP=0x80 で無害 (SHIFT 再 down
  後も shift 保持を 'A' 出力で実証)。キー状態 sense (keystat.ref) を読むゲームは break→make が
  同一呼び出し内で完結するため無傷。**ブラウザ実機確認済み (ユーザー、2026-07-11)**: ①エディタ
  (VZ/JED) でカーソル/文字が押しっぱなしでリピートする ②東方/Super Depth の押しっぱなし移動が
  滑らかなまま — どちらも問題なし。以下は当時の調査メモ (経緯):
...
- ✅ **起動 .bat の ② (線形列) 経路を ③ (文インタプリタ) に統合 — 完了 (2026-07-11、Exhibit B)**。
  .bat の起動を `buildStatements` → `qb_dos_stage_batch` の 1 本に統一し、② 専用の実体を全撤去:
  `resolveSequence` (batscript.js) / `stageAndRunScript` + `dosStageScript` cwrap (bridge.js) /
  `stageScript` (emu-worker.js + emu ファサード) / `np2kai_dos_stage_script` (bridge.c/h) /
  `qb_dos_stage_script` (dos_loader.c/h) / CMakeLists の export / ② ワイヤ形式 `"PATH\tARGS\n"`。
  Run ハンドラの routing は従来同値 (単一 cmd で set/cd も分岐も無い .bat は従来どおり ① 単一起動 =
  サブディレクトリ CWD 代行維持)。② の C API を直接使っていたハーネス 8 本
  (pmd/fmp/adpcm_beepgain/midi_serial/exec_env/batch_done の各 test + bio100_triage + mouse_chain_probe)
  も stage_batch (`C\t…`) へ移行。batscript_test の ② 検証 5 件は同等の ③ 検証に置換 (58/58 PASS)。
  **意図した挙動差 2 点**: (a) 線形複数 cmd .bat の `echo` メッセージが表示されるようになった
  (③ は元から表示 = 実 DOS と同じ・統一で線形にも適用) (b) `call`/`for` 等入り .bat は行 skip でなく
  ① 単一起動へ退避 (honest fallback に寄せた)。回帰 = run_tests.js 全通し + bio100_triage --fresh
  ベースライン一致で確認。同類の Exhibit A (argv[0] 整形を `format_argv0` に集約) は 2026-06-23 対応済み。
...
- **triage を改修** (`tools/bio100_triage.js`): ①ランチャ型は `.bat` を batscript.js で解釈し
  `stage_batch` 経路 (ドライバ常駐込み。2026-07-11 に旧 stage_script から統合) でステージ ②最終 PC を **EXIT(0xFEE30 正常終了)/
...
- 既存マウス入力 (Pointer Lock) との接続 → **INT 33h HLE ドライバとして実装済 (2026-07-03、MS/NEC 二流派・既定 MS。直接 I/O はそのまま)**
