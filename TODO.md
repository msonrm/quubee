...
## 🔧 進行中: 音声スレッド再設計 (emulator を Web Worker へ、2026-06-17〜)

FM 音楽の「テンポの揺れ・フレームが詰まるスキップ」を根治する。`qbDebug.audioStats()` の実機計測で
真因を確定 = **音声配信でなく emulation の cadence** (emulator が rAF 律速でメインスレッド飽和 +
サブ25msジッタ)。コア載せ替えでも過負荷対策でも直らず、**emulator を専用スレッドへ**が唯一の根治。

- 計画書 (合意済): [docs/audio_worker_migration.md](docs/audio_worker_migration.md)
- 前提: emulator 接点は `bridge.js` のみ・他5ファイルは純粋 → **Wasm 再ビルド不要の JS 再編成 + COOP/COEP**。
- **Stage 0 (地ならし、着手中)**: cross-origin isolation。`web/_headers` + `tools/devserver.js`。
  `crossOriginIsolated===true` と SAB 可用を確認。挙動不変。
- Stage 1: emulator→worker (OffscreenCanvas 描画 + 入力/FS marshaling、音は仮配線)。
- Stage 2: SAB リング + AudioWorklet + 需要クロック = **根治**。Stage 3: 仕上げ。
- 計測/balance ツールは実装済: `qbDebug.audioStats()` / `qbDebug.vol()` (`tools/vol_test.js`)。
