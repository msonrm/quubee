...
static void vram_clear_all(void) {
    /* 全 cell を space + 通常 attribute で埋める (BIOS INT 18h, AH=16h 相当)。
     * default attribute は 0xE1 (NP2kai bios_memclear の実機相当初期値と同じ)。 */
...
static void csi_dispatch(uint8_t final) {
    ...
    case 'm': /* SGR (色/属性) は無視。同色ベタ描画でも操作は可能。 */ break;
    case 'h': case 'l': {
        /* CSI mode set ('h') / reset ('l')。"ESC [ > n h/l" は PC-98 ANSI 拡張で
         * console driver が解釈するハードウェア制御コード。
         *   > 1 h/l  : カーソル表示 ON/OFF (今は no-op で OK、描画への影響なし)
         *   > 5 h/l  : **テキスト面表示 ON/OFF** ← NP2kai の GDC STOP/START と等価
         * gdcs.textdisp の GDCSCRN_ENABLE (0x80) を操作する。 */
        int set = (final == 'h');
        int n = csi_param(0, 0);
        if (g_csi_priv == '>' && n == 5) {
            if (set) {
                gdcs.textdisp |= GDCSCRN_ENABLE | GDCSCRN_ALLDRAW2;
            } else {
                gdcs.textdisp &= ~GDCSCRN_ENABLE;
            }
            fprintf(stderr, "[tty] CSI >5%c: textdisp now 0x%02x\n",
                    final, gdcs.textdisp);
        }
        /* それ以外の mode は無視 */
        break;
