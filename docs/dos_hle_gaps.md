...
10. AH=09h は `$` が無い場合 4096 byte で打ち切り。AH=0Ah のバッファ満杯は BEEP せず黙殺。
    ...
    **DOS CON ワークエリア `0000:0711h`（fkey 行表示状態）/`0712h`（テキスト行数−1）を維持**（2026-06-11）—
    起動時 0/24（fkey 非表示・25 行。我々はファンクションキー行を描画しないのでこれが正直な状態）に初期化し、
    私用シーケンス **`ESC[>1h/l`（fkey 行 非表示/表示）・`ESC[>3h/l`（20/25 行）** で追従更新する。
    master.lib（`TEXT_HEIGHT`/`text_fillca`）はこのバイトを**直読み**するため、未初期化（=0）だと全画面
    fill が 1 行で切れる＝**東方が黒反転セルで隠す VRAM タイルキャッシュ（右端 64×400px）が露出**していた真因。
    なお `ESC[>5h/l` は現状「テキスト面表示 ON/OFF」と解釈しているが master.lib では**カーソル表示制御**
    （TEXT_CURSOR_HIDE/SHOW）— 不一致の解消は要再調査。20 行モードは bookkeeping のみで tty 描画自体は
    25 行のまま。
