...
7. ~~**Ctrl-C / Ctrl-Break を検出しない**~~ → ✅ 2026-07-05 **INT 23h (Ctrl-C ハンドラ) 発火を実装**
   （Stosstruppe 氏 X 投稿 fig5.c/fig5.asm の指摘）。cooked コンソール入力 (AH=01/08/0Ah/0Bh/3Fh handle 0)
   が ^C (03h) を見たら "^C"+CRLF をエコーして INT 23h を発火（AH=06/07h と raw モードは見ない =
   実 DOS 契約）。既定ベクタは INT 20h トランポリン = プログラム中断（AH=4Dh type 1）。ユーザー
   ハンドラ (AX=2523h) は復帰先を INT 21h トランポリンの NOP にした INT フレームで呼び、実 DOS と
   同じ SP 規律で戻り方を判別: IRET = 呼び出し再開 / far RET = CF=1 なら中断・CF=0 なら再開。
   ブラウザ側も Ctrl+C キーだけゲストへ透過（bios09 CTRL バンクが 0x03 に変換）。回帰
   `tools/int23_test.js`。**残る差異**: Ctrl-Break（STOP キー）は別系統で未対応・BREAK ON
   （非コンソール関数でのチェック、33h は値保持のみ）は無し・レジスタを壊すハンドラの再開は不定
   （実 DOS も同様だが DOS 側は自前退避を持つ）・ハンドラ発火中の再帰発火はしない。
...
18. **PSP フィールドの欠落一式**（`build_psp` / EXEC 子共通）:
    ...
    - ~~**+0Ah/0Eh/12h（INT 22h/23h/24h 保存）を EXEC 時に保存せず、終了時の復元も無い**~~
      → ✅ 2026-07-05 実装（INT 23h 発火実装と同時）。PSP 構築時（最上位/EXEC 子とも）に現在の
      IVT[0x22..0x24] を保存し、終了時（4Ch/INT 20h/TSR）に IVT へ復元 — 子が AH=25h で差し替えた
      ^C ハンドラを終了後に残さない。
