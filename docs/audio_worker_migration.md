...
- [ ] **制御/デバッグ** — enableMidiNow (run flow) / setFmgen・setVol・各 qbDebug cwrap / qbDebug.fs
- [ ] **描画ループ + 音声** — モード固有 (local=rAF+ScriptProcessor / worker=OffscreenCanvas+SAB)。
      `emu.start()` に分離 (closure の frame() 駆動と onaudioprocess を各実装へ)。これが最後の山。
- [ ] **worker emu 実装** — emu-worker.js + call() を emu インターフェースに繋ぐ。
- [ ] **切替** — worker パスを setupApp(workerEmu) に → 旧 initWorkerMode/最小ドロップ撤去。
      最後に従来パスを setupApp(localEmu) へ (= 単一コード化) + 公開前の下げ側適応クロック。
