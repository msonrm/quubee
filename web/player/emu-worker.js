...
// スキップしイントロを削らない。g_musicStarted は stageMusic (新セッション確立) で倒す —
// 初回パスは stageMusic → musicPlay(曲A) → reset の順なので reset で倒すと最初の差し替えが素通りする。
...
onmessage = (ev) => {
    ...
    switch (m.type) {
        ...
        // 注: g_musicStarted はここでは倒さない。音楽セッションの初回パスは stageMusic →
        // musicPlay(曲A) → reset の順なので、reset で倒すと曲 B への最初の差し替えで
        // clearRing+swap-silence が発火しない (off-by-one)。倒すのは stageMusic 側。
        case 'reset': audioActiveReported = false; g_swapSilence = 0; c.reset(handle); reply(m.id, { ok: true }); break;
