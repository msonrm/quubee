...
- 検証: FEP/keymap 系 6 本 + **全体回帰 79 本 PASS**。CREDITS.md の pin 記載を更新。
  **ブラウザ実機 5 点 (§4-2: J 押しっぱなし濁音連打 / F+G→H+J 復帰 / 合成中 V+M 即確定 /
  F+G 直後ロールオーバー / Phase 2 BS = よみ戻し) もユーザー確認済・問題なし (2026-07-19)。
  同日 push + Cloudflare Pages デプロイ・本番アセット検証済 (0.12.0 / 1.4.0 / mutual) = クローズ**。
...
- 検証: quubee_run_test 10 + mcp_server_test 18 + stage_diskimage_test 8 = PASS (回帰は
  ...
  npm README の実出力 JSON も再採取 (int21Calls の実物入り。捏造なし)。**同日 publish 完了
  (ユーザー、latest=0.4.0)** — レジストリ実体を install し直して 18 PASS + 新スキーマ確認済。
  **MCP 関連の宿題はゼロ**。

## [MCP B2+D — ディスクイメージ入力 + 利用例ドキュメント + 外向き文面のスモークテスト化 = quubee-mcp 0.3.0] — 2026-07-17

MCP 計画の残り 2 段 (B2・D) と、ユーザー指摘の用語修正をまとめて 0.3.0 に載せた
(0.2.0 は同日 publish 済・レジストリ版検証済)。**0.3.0 も同日 publish 完了 (ユーザー)** —
レジストリ実体 (latest=0.3.0) を install し直して回帰 18 PASS + 配布物 CLI で .hdm 起動を
確認済。**MCP 計画 (A/B1/B2/C/D) 全段完了・全段レジストリ版検証済**。
...
そのまま調査に効いた** 好例。FD は「ドライブの指定が違います」で停止していた。トレース（`int21_trace`）と
`int21Unimplemented` で 4 つの穴を順に特定・解消:

1. **IOCTL `AH=44h AL=09h`（drive remote?）未実装** → FD は全 26 ドライブを列挙して有効判定するが、A:
   すら CF=1 で「無効」判定 → 停止。**A: を bit12=1（リモート）で返す**ことで解決。faithful でもある —
   我々の A:(/run) は実 FAT を持たないホスト連携 FS で、ローカル固定ディスクより network/CD 相当。
2. **リモート判定が本質的な鍵だった**（fd98_313.doc のヒント）: FD はローカルだと「ディレクトリを
   直接セクタ読み」する経路で、実 FAT の無い我々では一覧を作れず空白停止。リモートだと ver3.12 で
   追加された「DOS ファンクション（FindFirst）経路」に切替 → 実装済み INT 21h で一覧が出る。
3. **AH=32h（Get DPB）** は **AL=0FFh（DPB 無し）** に。リモートドライブにローカル DPB は無い（実 DOS も
   同様）。合成 DPB を返すと「リモートなのに DPB がある」矛盾で FD が直接セクタ経路に迷い停止した
   （DOS 4.0+ 形の合成 DPB → FD が AH=00 終了、DOS 3.x 形 → 停止、を経て「DPB 無し」で正解に到達）。
4. **AH=60h（TRUENAME）** と **AH=37h（Get/Set Switch Char）** を faithful 実装（前者は `read_dos_rel`
   再利用で "A:\PATH" 大文字化・DBCS 保護、後者は switch='/'・availdev=FF）。

結果: FD がバージョンヘッダ・空き容量・ファイル一覧（FD98.COM 等）・ファンクションキーメニュー
（Logdsk/eXec/Copy/…/Unpack）まで完全描画し、ブラウズ可能に。**閲覧主体の我々のスコープと一致**
（直接書換系 `[Write]`/`[Sort]` はリモートでは FD 自身が無効化）。
検証: スイート全 75 本 PASS（新設 `tools/fd_filer_test.js` 6 項目含む）+ **bio100 triage --fresh が
ベースライン完全一致**（描画到達 25・動作確認 27・EXIT 0・CRASH 0 = 「A: リモート化」はゲーム corpus に
無影響。元々 AH=32/AL=09 は未実装で全通ししていた corpus なので依存ゼロ）。正典 = docs/dos_hle_gaps.md
§1・§2.5。**ブラウザ実機確認済み (ユーザー、2026-07-12): ファイル一覧・ドキュメント閲覧が動作。
同梱のテキストエディタ・音楽プレイヤー等も普通に動く** → デプロイ済み。
...
通っている。64KB 固定の寛容さに依存する公開ソフトは存在しないはず」。裏付けもリポジトリ内に
あった — EXE 側は既に実 DOS 相当 (e_maxalloc=FFFF → 全所有) で、寛容だった旧実装を忠実化した
とき壊れるどころか brpn が直っている (dos_loader.c:1743 のコメントに記録済み)。

変更 = `dos_loader.c`: `qb_dos_alloc_reset` に「空きアリーナ無し = プログラム本体が終端 Z
ブロックとして 0xA000 まで所有」形を追加し、COM ステージの arena base を `+0x1000` (64KB) から
`MEM_TOP` へ。これで ①シュリンク無しの AH=48h/EXEC は実機同様に失敗 (largest=0) ②64KB 超を
黙って使う CP/M 流の合法ソフトがアリーナ MCB を踏み潰さなくなる (size=0xFFFF ガード事件の根)
③PSP:2 (0xA000) と MCB の答えが一致。初回 self-shrink → `alloc_reset` 再構築の既存機構
(EXE が毎回通る道) がそのまま受ける。

検証 = ①スイート全 72 本 PASS (自前の合成テスト COM も寛容さに無依存と判明) ②bio100 triage
--fresh を新旧 wasm で回した**タイトル単位 A/B: 31/31 本 tier/state 完全一致** (公開 corpus に
依存ゼロを実証。唯一の COM ゲーム SEENA2 も RENDER 232 色で不変) ③新設回帰
`tools/com_memown_test.js` (5 項目・素材不要で CI 可): シュリンク前 largest=0 / シュリンク後に
空き出現。**旧 wasm に掛けると largest=0x8EFF を検出して FAIL = 検出力も実証**。
ブラウザ実機スポットもユーザー確認済み (2026-07-12、「普通に動く」) → デプロイ済み。
...
を A/B 実測して**不採用と決着**。手順 = compile/link 両段にフラグ追加 → 3 ベンチ各 3 回 +
Suika3 は wasm 差し替えインターリーブ 3 対 (ドリフト相殺):

| ワークロード | baseline | =wasm | 判定 |
|---|---|---|---|
| bench_frame (例外連発型) | 18.02 ms/f | 10.60 ms/f | 1.70 倍改善 |
| bench_game (Suika3 32bit) | 7.98 ms/f | 8.16 ms/f | **約 3% 悪化 (6/6 回重なりゼロ = 実コスト)** |
| bench_ray (16bit) | 9.63 ms/f | 9.57 ms/f | ノイズ域 |

改善が出るのは CPU 例外を連発するワークロード (FreeDOS ブートスピン型) だけで、これはサポート外。
主力の 32bit ゲーム級 (DOS/4GW) に実害が出る上、Wasm EH は古いブラウザ (Safari 15.2 未満等) を
切る。screen hash / audioRms は全一致 = 挙動不変の確認のみ収穫。CMakeLists は素に戻し、成果物も
ベースライン (SHA 5db22ace…) を復元。例外連発する実タイトルが現れたら再考する。
TODO.md「エミュ本体の高速化」に記録。**これで patch 07 から続いた速度パスは一区切り。**
...
('A' 大文字で実証)。回帰スイート 70/70 PASS。ブラウザ実機確認済み (ユーザー、2026-07-11:
①VZ/JED のリピート ②東方/Super Depth の押しっぱなし移動が滑らかなまま — どちらも問題なし)。
...
ブラウザ実機 (線形複数 cmd .bat のゲーム起動) もユーザー確認済み (2026-07-11、問題なし)。

## [既定クロックを multiple=27 (≈66MHz) へ引き上げ] — 2026-07-11

第 2 弾のブラウザ実機確認 (ユーザー): **Ray で multiple 38 まで音が持つ (39 からプチノイズ)**。
第 1 弾後の 26 → 38 で、素の頃 (20 未満でノイズ) からホスト余裕は約 2 倍になった。
トレードオフ (vsync 同期ゲームは速度不変で負荷だけ増える / 速度ガード無しの旧作は 1.35 倍速く回る、
対症 qbDebug.multiple(20)) を提示のうえ、**ユーザー判断で既定 27 を再採用** (bridge.js
`DEFAULT_MULTIPLE`、JS のみ・Wasm 不変)。過去 2 回 (2026-06-14/27) の「27 で音詰まり」は
ホスト律速が真因で、patch 07 により解消していたことが確定した形。headless (machine.js) の既定は
回帰テストの暖機フレーム数が 20 前提のため 20 のまま (bench 系は multiple を明示引数で取る)。
...
明確で場当たりではないが、その隣で毎回ゼロから書かれる**調査用ハーネス**に、そのつど微妙に違うバグが
入り込んでいた。今回の Suika3 追跡で実害を 3 件踏んだ:

1. 音声を「1 エミュフレーム分」の端数サンプルで汲んだ。`qb_audio_fill` は `frames < bufsize` でも
   `sound_pcmlock()` が **1 ブロック丸ごと生成・消費**するので、音が約 5 倍速で進み PI 割り込みが
   5 倍に見えた。**存在しないエミュのバグを半日追いかけた**。
2. ヘッダだけ直したら ninja が再ビルドせず (Watcom は depfile を出さない)、**古い wasm を測って**
   結論を出しかけた。2 回起きた。
3. 「BGM が鳴る場面」まで毎回 2500〜3000 フレーム空回し。1 回 60〜250 秒 × 20 回近く。

`tools/lib/machine.js` で 3 つとも型で封じた。標準化 (①ブート手順の集約) より一段強い部分がある:
**間違ったやり方を表現できなくする** (②③)。

- `Machine.boot / runFrames / runUntil / pressKey / textVram / screenshotPng / screenHash /
  captureAudio / int21 / xms / exited / batchDone`
- **`captureAudio()` は必ず `np2kai_audio_get_bufsize()` ちょうどで汲む** (罠 1 を封じる)
- **`info()` が wasm の SHA/mtime/設定/フレーム番号を必ず返す** (罠 2。素性の分からない数字を出さない)
- **`snapshot()` / `restore()`** (罠 3)。**暖機 17s → 復元 0.07〜0.43s = 40〜200 倍**（復元時間はばらつく）

### snapshot の境界を falsify で確定させた

「Wasm の線形メモリだけで足りるはず」は**誤り**だった。最初のテストは画面・テキスト・音声がずれて FAIL。
libc の `FILE*` は heap に載るが、その先の **fd → ノード/読み込み位置 の対応は Emscripten の JS 側
(`FS.streams`)** に居る。Suika3 は `assets.arc` を位置 266563 で開きっぱなしにしており、ヒープだけ戻すと
復元後の最初の `fread` が壊れ、そこから全部ずれる。正しい境界は:

> **Wasm 線形メモリ + MEMFS のファイル + 開いているストリーム表 (`FS.streams`) + cwd**

- 検証 = `tools/snapshot_test.js`。連続実行の続きと、別モジュールへ復元した続きが、画面の全画素ハッシュ・
  テキスト VRAM・`INT 21h` カウンタ・**音声の中身 (1 サンプルまで)** で一致することを確認。
- wasm の SHA が違う snapshot は**復元を拒否**する (関数テーブルの索引はビルド固有)。
- 罠: `growHeapTo` は `malloc`→`free` を繰り返すと、解放した空きブロックが次の要求を満たして heap が
  伸びず無限ループになる。目標に届くまで確保したまま積み、最後に解放する。

### 残

既存 `tools/*.js` の移行は**一気にやらない** (回帰 69 本は「何も壊していない」ことを教える唯一の装置で、
まとめて書き換えると主張が静かに変わったことに気づけない)。触るスクリプトから 1 本ずつ。
次 = `bench_frame.js` の移行 → エミュ本体の高速化 (A/B を何十回も回すので snapshot が効く) →
snapshot の圧縮 → headless CLI → MCP アダプタ。**MCP は「参照プラットフォーム」ではなく
「煙感知器と計測器」として出す** (HLE-DOS は実 DOS ではなく、動いたことは実機動作を保証しない)。
...
- 報告: 長い未確定文字列が画面右端で見えなくなり、確定して初めて全文が見える
  (M1 の既知制限「行末打ち切り」が実運用で顕在化)。
- 対応 (native/dos_fep.c): 表示をカーソル起点の**線形セル範囲** (VRAM は 80 桁×行の線形
  メモリ) に一般化し、行末で次行へ折り返す (実 FEP と同じ)。全角が行末 1 桁に残る場合は
  同属性の空白 1 セルでパディングして次行頭から描く (tty の折り返し規律 = 漢字を左右に
  割らない)。画面末尾を越える超長文は先頭から削って**末尾優先表示** (打っている場所が
  見える)。退避・復元・セル所有権検証も同じ線形範囲でそのまま機能する。
- 回帰: fep_test.js に wrap (複数行復元) / straddle (行末またぎ全角のパディング) /
  tail (末尾優先・画面最終セルまで使用) の 3 ケース 12 チェックを追加、計 29 全 PASS。
  既存全スイート green。
...
### タイムゾーン注入 — 「いま」が 9 時間ずれる根治 (同日追記 6)

- 報告: 「いま」の変換で 9 時間前の時刻候補が返る (「きのう」の日付候補も 1 日ずれ)。
- 真因: wasm に zoneinfo が無く TZ も未設定のため、mozc の日時リライタが使う absl/cctz の
  local_time_zone() が UTC にフォールバック (JST = UTC+9)。POSIX 形式 (JST-9) の TZ も
  zoneinfo 不在では解決できない。
- 対応: cctz が zoneinfo 無しで合成できる固定オフセットゾーン名 **`Fixed/UTC±hh:mm:ss`**
  (time_zone_fixed.cc FixedOffsetFromName) をホストの Date オフセットから組み立て、
  mozc-worker.js の preRun で ENV.TZ に注入 (リンクに ENV export 追加)。
  実測: いま → 午前6時36分 (ホスト一致)・きょう → 2026年7月8日 水曜日 ✓。
...
**デプロイ済 (2026-07-05)**: commit 310c484 を push + wrangler で本番反映。wasm md5 が
local/プレビュー/本番で一致 (bd01339e…)。
...
**デプロイ済 (2026-07-05)**: JS 側 4 件 (b3c35b3) + 本 4 点 (bce51df) を push + wrangler で
本番反映。wasm md5 が local/プレビュー/本番で一致 (73adfeeb…)、JS マーカ (releasePadKeys /
stageMusic 移動) も本番 URL で確認済。
...
### 実装 (JS/HTML/CSS のみ)
設定パネルに "Gamepad / ゲームパッド" グループを追加。
- **方向モード切替**: カーソル ⇄ テンキー 2468 (既定カーソル・斜めは同時押し)。WASD は PC-98 期の文化でなく
  ゲームも読まないため非採用 (ユーザーと確認)。
- **ボタン→キー割当**: A(0)/B(1)/X(2)/Y(3)/L1(4)/R1(5)/Start(9) に動作キー (Z/X/C/Space/Enter/Esc/Ctrl/
  Shift/None) を dropdown で割当。矢印・Tab は候補から除外 (移動は方向モードが担う)。
- **押下ボタンの live 表示**: パネル表示中はパッドのボタンを押すと該当行が光り「今押したボタン」を表示
  (ラベル無しパッドでも物理ボタンを特定できる)。pollGamepads はパネル中は live 検出のみでゲームへ送らない。
- 割当は quubee_settings_v1 の `pad` に保存・起動時復元。反映は即時。

### 中核 = pollGamepads の if→テーブル化 (ゼロ回帰)
button→NKEY の if 文を PAD_BUTTONS/PAD_KEYS/PAD_DIRS/padBtnMap のテーブルへ。**既定値を現行 if と 1:1** に
した上で、旧マッピングと新テーブルの `want` 集合を全ボタン組合せ (14336) で突合し**不一致 0 を確認** =
既存ユーザーのパッド操作は完全不変。方向モードは PAD_DIRS で切替 (テンキー 8/2/4/6=0x43/0x4b/0x46/0x48)。

### 検証
node --check OK・挙動同一性 14336/0・新 id 存在・タグ均衡・native 無変更。実機 (コントローラ) 確認はユーザー。

### 備考: 案B (ネイティブ PC-98 joyport) は保留
PC-9801-86 の ATARI 仕様ゲームポートを直読みするゲーム向けの「案B」は、SUPPORT_GAMEPORT は ON だが
joymng_getstat=0xff 固定 (ホスト供給未配線)。需要 (joyport-only の corpus タイトル) が出たら別途 C 側で実装。
...
### 追補 (同日、デプロイ済)
- 別窓ビューア (readme/テキストの拡大プレビュー #viewer-*) をライトモード時に .M ダイアログ (#player-box) と
  同じ紙トーンへ (:root[data-theme=light] 限定の色上書き・文字サイズ/行間は据え置き・ダークはゼロ回帰)。
- 「次の Run から」バッジを英語併記に (「次の Run から / Next Run」・全 5 箇所)。
- マウス流派に **None (ドライバ無し)** を追加 (ユーザー要望)。あわせて `qbDebug.mouse33` が `'off'` 等の真値
  文字列で `mode ? 1 : 0` により誤って MS に落ちていたバグを修正 (`'off'`/`'none'` を明示的に 0=無効へ・
  ベース/worker 両方・数値経路は温存)。単体検証 8/8。
...
- **円弧は始点 s から終点 e へ CCW** (N88-BASIC CIRCLE の向き)。パイ放射線は flag bit2=中心→s / bit3=→e。
  ※初回デプロイは回転向きを逆 (CW) に取り扇が上下反転していた (報告者の実機指摘) → 同日 s→e CCW へ是正。
...
**ブラウザ実機確認済 (2026-07-03、ユーザー)**: MIMPI v3.8 + KNGNACHT.mid が I/F = MPU で
MIDI 演奏 + ミキサー画面フル表示。LIO 根治・ディレクトリ open 拒否・MIDI 拡張子
ヒューリスティックと合わせて 4 点セットで解決。
...
### 背景
SimK 氏の TEXTTEST 更新版 (第3版)。既対応の 8 本に加え **PC98RET (戻り値検証) /
PC98WORK (ワークエリア 0060:0107-013F 全域テスト) / PC98WDOS (ワーク直書き→DOS 出力)**
の 3 本が新規。今回は np21w の実行結果が実機と突合済みで、**DOS 3 と DOS 6 の両リファレンス**
付き。3 本を headless で全ページ突合した。

### 結果
- **PC98WDOS: 全 9 テスト最初から一致** (0710/071C の load→store 双方向同期が全出力経路で有効)。
- **PC98RET: AH=02h/06h/09h が AL を返していなかった** → 実 DOS 契約 (02h: AL=出力文字・
  TAB は展開スペースの 20h / 06h: AL=DL / 09h: AL=24h) を実装。他ページ (AH=40h CF/AX・
  分割 ESC・INT DCh) は元から全一致。
- **PC98WORK: 世代差の発見と 4 ギャップの根治** (DOS 3.x は byte スロット 011D/0114 が真実で
  ESC[u は 0726/0727/072B を毎回 live 読み。DOS 6.x は word スロット 013C/013E + one-shot 化。
  我々は live 同期=ワークエリアが真実という設計と同型の **DOS 3.x 意味論で統一**):
  1. 起動時既定値 (0714=E1・0119=20・071B=1・072B=E1・013C/013E=00E1) を実機 dump と一致させた
  2. 消去系 (ESC[J/K・cls) とスクロール新規行を「クリア文字 0719h + クリア属性 0714h」で
     埋める vram_put_clear に統一 (現在属性でなく。SGR 色のままスクロールしても新規行は白 = 実機)
  3. 現在属性 071Dh を出力前に読み戻し (直書きが次の出力から効く)
  4. ESC[s/u の保存先をワークエリア 0726h/0727h/072Bh に (内部変数を廃止、ESC[u は live 読み)
- **スクロール範囲 011Eh/0112h の窓スクロールは意図的に見送り** (ユーザー判断、TODO.md に詳細)。
  既定値=全画面で通常は差が出ないが、直書きソフトは corpus 未遭遇・回帰面積が大きいため。

### 検証
- PC98RET 全 AL 一致 / PC98WORK PAGE1-5・7 が DOS3 リファレンスと一致 (PAGE3 は位置+属性の
  live 復元、PAGE4 は 011D 反映、PAGE5 は赤反転クリア埋めまで完全一致)。PAGE6 (スクロール窓)
  のみ既知の見送り。
- 新規回帰 `tools/conwork_test.js` (13 項目)。既存全スイート 53 本 PASS・bio100 triage も
  基準どおり (ALIVE=21/CRASH=0) = 回帰ゼロ。
...
- 副次効果: フルスクリーンエディタの点滅ハードウェアカーソル位置もこれで決まるため、
  既知課題「JED の点滅カーソルが左上に居座る」も同根で解消。**ブラウザ実機確認済み
  (2026-07-02、ユーザー: QB アプリ起動 + JED カーソル追従の両方を確認)**。
